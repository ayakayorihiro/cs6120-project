...
let rec codegen_expr expr = 
  match expr with
  | Input i -> unimplemented (* todo get a helper involved *)
  | FuncCall (name, args) -> unimplemented (* see Kaleidoscope guide *)
  | Plus (e1, e2) 
  | Subtract (e1, e2) 
  | Multiply (e1, e2)
  | Divide (e1, e2) 
  ...
  | Ternary (e1, e2, e3) -> unimplemented (* const_select? *)
...
let codegen_stmt s = unimplemented
