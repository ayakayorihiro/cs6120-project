...
let visit_action (Action (p, s)) = 
    ignore (Option.map visit_pattern p);
    ignore (Option.map (visit_stmt [] false false) s)

(* The entrypoint of the typechecking code. *)
let visit_program (Program (f, a)) = 
    ignore (List.map (fun (f, n) -> Hashtbl.add functions f n) builtin_functions);
    ignore (List.map (record_global []) builtin_variables);
    ignore (List.map record_func f);
